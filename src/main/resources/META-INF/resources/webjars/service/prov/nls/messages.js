define({
    'root': {
        ...
        'service:prov:instance-max-variable-cost-help': 'Maximal cost where this instance will be up. When undefined, no limit. Whenits liit is reached, this instance is terminated.',
        ...
        'service:prov:terraform:status': 'Started {{startDate}} by {{author}}{{#if toAdd}}, {{toAdd}} to add{{/if}}{{#if toChange}}, {{toChange}} to change{{/if}}{{#if toDestroy}}, {{toDestroy}} to destroy{{/if}} {{#if failed}}<i class="text-danger fas fa-exclamation-circle" data-toggle="tooltip" title="Failed {{endDate}} at <strong>{{command}}</strong> command<br>See logs for details"></i>{{else}}{{#if end}}<i class="text-success fas fa-check-circle" data-toggle="tooltip" title="Succeed {{endDate}}"></i>{{else}}<i class="text-primary fas fa-sync-alt fa-spin" data-toggle="tooltip" title="Pending <strong>{{command}}</strong> command..."></i>{{/if}}{{/if}}',
        'service:prov:terraform:status-generate': 'Generating Terraform resources', 
        'service:prov:terraform:status-init': 'Terraform <strong>init</strong> command', 
        'service:prov:terraform:status-plan': 'Terraform <strong>plan</strong> command', 
        'service:prov:terraform:status-apply-completed': 'Terraform <strong>apply</strong> {{[0]}}/{{[1]}} handled resources',
        'service:prov:terraform:status-apply-completing': 'Terraform <strong>apply</strong> {{this}} being handled resources',
